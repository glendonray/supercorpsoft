...
function gg_register_acf_blocks() {
    ...
    register_block_type( get_stylesheet_directory() . '/blocks/user-authentication' );
}
add_action( 'init', 'gg_register_acf_blocks' );


// Adding a new (custom) block category and show that category at the top
add_filter( 'block_categories_all', 'gg_custom_block_category', 10, 2);
function gg_custom_block_category( $categories, $post ) {
