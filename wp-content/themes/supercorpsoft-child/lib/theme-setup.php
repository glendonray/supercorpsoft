 ...
 *
 * @package WordPress
 * @subpackage Super Corp Soft Child
 * @since 1.0
 * @version 1.0
 */
...
if ( ! function_exists( 'gg_enqueue' ) ) {
	...
	function gg_enqueue() {
		 ...
		 */
		wp_enqueue_script( 'gg-head', get_stylesheet_directory_uri() . '/dist/js/head.min.js', array(), '1.0', false );

		/**
		 * Enqueue scripts in footer.
		 * This is a concatinated, minified file of scripts from ./src/js/footer/
		 */
		wp_enqueue_script( 'gg-scripts', get_stylesheet_directory_uri() . '/dist/js/scripts.min.js', array(), '0.0.2', true );
	}
}
...
if ( ! function_exists( 'gg_enqueue_block_editor_assets' ) ) {
	...
	function gg_enqueue_block_editor_assets() {
		wp_enqueue_style( 'gg-editor-styles', get_stylesheet_directory_uri() . '/dist/css/editor-styles.css', null, '1.0' );
	}
}
add_action( 'enqueue_block_editor_assets', 'gg_enqueue_block_editor_assets' );

if ( ! function_exists( 'gg_override_mp6_tinymce_styles' ) ) {
	/**
	 * Add custom styles to ACF wysiwyg editor.
	 */
	function gg_override_mp6_tinymce_styles( $mce_init ) {
	...
	add_filter( 'tiny_mce_before_init', 'gg_override_mp6_tinymce_styles' );
}
